...
More layers= more key and value vectors need to be stored = increasing the KV Cache size.

- T: Number of Tokens
The KV Cache stores key and value vectors for every token in the prompt and every token in the answer by the model.
For example:

Prompt = 1,000 tokens
Generated response = 500 tokens

Total tokens stored in the KV Cache(1000+500) = 1,500 tokens

As the number of tokens increases =  KV Cache size also increases.

For example:
2× more tokens = around 2× more KV Cache memory

- H_KV: Number of KV Heads
H_KV =number of key and value heads in each layer.
...
This means that the groups of **query** heads share the same key and value heads.

Fewer KV heads =fewer key and value vectors need to be stored =  reducing the KV Cache size.
